...
def products():
    with tracer.start_as_current_span("catalog.products.query") as span:
        ...
        log_event("products", "produtos listados", {"count": len(result)})
...
def product(product_id: str):
    with tracer.start_as_current_span("catalog.product.query") as span:
        ...
        if row is None:
            ...
            log_event("product-not-found", "produto não encontrado", {"product_id": product_id})
            raise HTTPException(status_code=404, detail="produto não encontrado")
        log_event("product-hit", "produto consultado", {"product_id": product_id})
